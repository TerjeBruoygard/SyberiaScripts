modded class FireplaceBase
{
	...
	override protected void TransferHeatToNearPlayers()
	{
		...
		GetGame().GetObjectsAtPosition ( GetPosition(), PARAM_HEAT_RADIUS, nearest_objects, NULL ); 
		
		ItemBase item;
		foreach (Object nearest_object : nearest_objects)
		{
			if (nearest_object != this && nearest_object.GetParent() == null && ItemBase.CastTo(item, nearest_object))
			{
