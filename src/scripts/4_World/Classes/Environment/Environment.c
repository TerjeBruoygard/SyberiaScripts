...
{
	bool m_isWinter = false;
	float m_clothingHeatIsolationGlobal = 1;
	...
	override void Init(PlayerBase pPlayer)
	{
		...
		m_clothingHeatIsolationGlobal = GetGame().ConfigGetFloat("CfgWorlds " + worldName + " clothingHeatIsolationGlobal");
		if (GetGame().ConfigGetInt("CfgWorlds " + worldName + " winterWorld") == 1)
		{
			m_isWinter = true;
		}
	}
			...
			if ( attachment.IsClothing() )
			{
				...
				for ( int i = 0; i < pBodyPartIds.Count(); i++ )
				{
					if ( attachmentSlot == pBodyPartIds.Get( i ) )
					{
						...
						float heatIsoMult = m_clothingHeatIsolationGlobal;
